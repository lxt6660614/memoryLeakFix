...
HashTb::HashTb(unsigned int _low, unsigned int _address, Chunk* _next, Chunk* _prev){
    this->low = _low;
	this->address = _address;
    this->next = _next;
    this->prev = _prev;
...
unsigned int HashTb::type() { return HashTb; }

MemLeakFix::MemLeakFix(){
    this->mem_arbt.clear();
    this->ptr_arbt.clear();
}

void MemLeakFix::allocate(unsigned int mem_address, unsigned int mem_size){
    #ifdef MemLeakFix_DEBUG
    cerr << "allocate: " << hex << mem_address << " " <<  mem_size << endl;
    #endif
...
// void MemLeakFix::link(unsigned int ptr_address, unsigned int mem_address, unsigned int mem_size){
//     #ifdef MemLeakFix_DEBUG
...
void MemLeakFix::link(unsigned int ptr1_address, unsigned int ptr2_address){
    #ifdef MemLeakFix_DEBUG
...
void MemLeakFix::remove(unsigned int address){
    #ifdef MemLeakFix_DEBUG
...
void MemLeakFix::fix(unsigned int address){
	#ifdef MemLeakFix_DEBUG
    cerr << "allocate: " << hex << ptr_address << " " <<  mem_address << " " <<  priv << endl;
    #endif
...
bool MemLeakFix::check_dup(Mem* mem){
...
bool MemLeakFix::check_mem_exist(unsigned int mem_address, unsigned int mem_size){
...
bool MemLeakFix::check_ptr_exist(unsigned int ptr_address){
...
void MemLeakFix::link_ptr_chunk(unsigned int ptr_address, Chunk* chunk){
...
void MemLeakFix::unlink(Ptr* ptr){
...
Chunk* MemLeakFix::get_last_chunk(Chunk* chunk){
    Chunk* _chunk = chunk;
    for(; _chunk; _chunk = _chunk->next);
    return _chunk;
}

Chunk* MemLeakFix::get_first_chunk(Chunk* chunk){
    Chunk* _chunk = chunk;
    for(; _chunk->type() != MEM; _chunk = ((Ptr*)_chunk)->prev);
    return _chunk;
}

void MemLeakFix::remove_mem(Mem* mem){
